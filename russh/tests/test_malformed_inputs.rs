...
#[tokio::test]
async fn subsystem_request_with_trailing_bytes_rejected_by_server() {
    let result = tokio::time::timeout(
        Duration::from_secs(3),
        raw_pty_req_signal(|server_channel| {
            let mut payload = channel_request_payload(server_channel, b"subsystem");
            encode_string(&mut payload, b"sftp");
            payload.push(0);
            payload
        }),
    )
    ...
    assert!(
        ...
        "server accepted a subsystem request with trailing bytes: {result:?}"
    );
}

#[tokio::test]
async fn shell_request_with_trailing_bytes_rejected_by_server() {
    let result = tokio::time::timeout(
        Duration::from_secs(3),
        raw_pty_req_signal(|server_channel| {
            let mut payload = channel_request_payload(server_channel, b"shell");
            payload.push(0);
            payload
        }),
    )
    ...
    assert!(
        ...
        "server accepted a shell request with trailing bytes: {result:?}"
    );
}
...
#[tokio::test]
async fn x11_request_with_trailing_bytes_rejected_by_server() {
    let result = tokio::time::timeout(
        Duration::from_secs(3),
        raw_pty_req_signal(|server_channel| {
            let mut payload = channel_request_payload(server_channel, b"x11-req");
            payload.push(0);
            encode_string(&mut payload, b"MIT-MAGIC-COOKIE-1");
            encode_string(&mut payload, b"0123456789abcdef");
    ...
    assert!(
        ...
        "server accepted an x11 request with trailing bytes: {result:?}"
    );
}
...
#[tokio::test]
async fn agent_forward_request_with_trailing_bytes_rejected_by_server() {
    let result = tokio::time::timeout(
        Duration::from_secs(3),
        raw_pty_req_signal(|server_channel| {
            let mut payload =
                channel_request_payload(server_channel, b"auth-agent-req@openssh.com");
            payload.push(0);
            payload
        }),
    )
    ...
    assert!(
        ...
        "server accepted an agent-forward request with trailing bytes: {result:?}"
    );
}
